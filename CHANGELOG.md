...
### Security
- Updated PHP and JavaScript dependencies to resolve reported security advisories (#113).
...
### Fixed
- `should_intercept_query()` now validates that all public searchable post types are indexed before intercepting generic searches, preventing silent exclusion of unindexed CPTs.

### Changed
- Added DocBlocks for methods in search hooks, token service, and indexer classes.
