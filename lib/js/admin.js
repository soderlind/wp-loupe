...
class WpLoupeFieldManager {
	constructor() {
		...
		this.postTypeSelect = document.getElementById(
		...
		this.previouslySelectedPostTypes = Array.from(
			this.postTypeSelect?.selectedOptions || []
	...
	async startBatchedReindex() {
		...
		if ( selectedPostTypes.length === 0 ) {
			this.showStatusMessage(
				this.__( 'No post types selected for indexing.', 'wp-loupe' ),
				'error'
			);
			return;
		}
		...
		try {
			...
			while ( true ) {
				const response = await wp.apiFetch( {
					path: 'wp-loupe/v1/reindex-batch',
					method: 'POST',
					data: {
						reset: isFirst,
						cursor,
			...
			this.showStatusMessage(
				`${ this.__( 'Reindex failed', 'wp-loupe' ) }: ${
					error.message
				}`,
				'error'
			);
	...
	initializeSelect2() {
		...
		jQuery( this.postTypeSelect )
			.select2()
			.on( 'select2:select select2:unselect', async ( e ) => {
				const eventType =
					e.type === 'select2:select' ? 'adding' : 'removing';
				const postType = e.params?.data?.id;

				if ( ! postType ) {
					return;
				}
				const previouslySelectedPostTypes = [
				...
				if ( eventType === 'removing' ) {
					...
					eventType === 'adding' &&
					! previouslySelectedPostTypes.includes( postType )
				) {
	...
	createSortDirectionSelect(
		...
		const options = [
			{
				value: 'asc',
				text: this.__( 'Ascending', 'wp-loupe' ),
				selected: savedFieldSettings.sort_direction === 'asc',
			},
			{
				value: 'desc',
				text: this.__( 'Descending', 'wp-loupe' ),
				selected:
					isDateField ||
	...
	preventFormSubmission() {
		...
		form.onsubmit = ( e ) => {
			...
			if ( e.submitter && e.submitter.type === 'submit' ) {
				this.showStatusMessage(
					this.__( 'Saving settings…', 'wp-loupe' ),
					'info'
				);
