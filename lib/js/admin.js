...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Track the previously selected post types to identify newly added ones
    let previouslySelectedPostTypes = Array.from(postTypeSelect.selectedOptions).map(option => option.value);

    // Prevent form submission
    const form = postTypeSelect.form;
    if (form) {
    ...
    // Initialize Select2 and handle its change
    jQuery(postTypeSelect).select2().on('select2:select select2:unselect', function() {
        const currentlySelectedPostTypes = Array.from(postTypeSelect.selectedOptions).map(option => option.value);
        
        // Identify newly added post types
        const newlyAddedPostTypes = currentlySelectedPostTypes.filter(
            postType => !previouslySelectedPostTypes.includes(postType)
        );
        
        updateFieldsConfig(newlyAddedPostTypes);
        
        // Update the tracking array for next time
        previouslySelectedPostTypes = [...currentlySelectedPostTypes];
    });

    async function updateFieldsConfig(newPostTypes = []) {
        ...
        for (const postType of selectedPostTypes) {
            try {
                ...
                if (Object.keys(fields).length > 0) {
                    // Pass whether this is a newly added post type
    ...
    function addFieldConfigSection(postType, fields, isNewPostType) {
        ...
        [
            __('Field', 'wp-loupe'),
            __('Indexable', 'wp-loupe'),
            __('Weight', 'wp-loupe'),
            __('Filterable', 'wp-loupe'),
            __('Sortable', 'wp-loupe'),
            __('Sort Direction', 'wp-loupe')
        ].forEach(text => {
        ...
        Object.entries(fields).forEach(([fieldKey, fieldData]) => {
            ...
            // For new post types or protected fields, always check the indexable checkbox
            ...
            // Filterable checkbox - enabled for date fields and newly added post types
            ...
            // Sortable checkbox - enabled for date fields and newly added post types
            ...
            ascOption.value = 'asc';
            ascOption.textContent = __('Ascending', 'wp-loupe');
            ascOption.selected = savedFieldSettings.sort_direction === 'asc';
            
            const descOption = document.createElement('option');
            descOption.value = 'desc';
            descOption.textContent = __('Descending', 'wp-loupe');
